...
```
데이터베이스에 sql문으로 검색하여 id중복을 확인한다. signup.html에서 응답으로 "중복있음" 또는 "중복없음"을 수신하면 결과에 따라 페이지를 업데이트한다.<br/><br/>
**<signup.html>**
```javascript
function updatePage(){
...
위의 과정이 아이디 입력 후 커서를 옮기면 비동기 방식으로 이루어진다.

### 어려웠던 점
- **3명간 협업** : 팀 프로젝트를 처음 시도해봐서 어려운 부분이 있었다. 특히 짧은 시간내로 완성해야 하는 프로젝트였으므로, 빠른 개발속도가 중요했다. 그러나 다른 팀원들이 초기에 다른 일정으로 인해 개발에 참여하기 어려웠기에 나는 빠르게 기본 틀을 기획하였다.<br/>
